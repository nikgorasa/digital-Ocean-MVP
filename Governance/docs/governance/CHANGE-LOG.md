...
| CRDB-GOV-013 | 2026-07-25 14:00 | System | CANCEL/TBO-ARCH/RECONCILE implementation — Cancel button in FlightBookingModal, ConfigProvider for flight endpoints, forceMock fallback, Zaakpay refund in cancellation, reconciliation cron for stuck PENDING bookings | src/components/FlightBookingModal.tsx, src/lib/tbo-flight-api.ts, src/lib/config-service.ts, src/lib/tbo-flight-client.ts, src/app/api/cancellations/route.ts, src/app/api/cron/reconcile-bookings/route.ts (new), Governance/docs/governance/SPRINT-PLAN.md | Revert git commit and restore governance docs |
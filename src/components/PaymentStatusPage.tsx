...
export default function PaymentStatusPage({ bookingId, orderId, isMock }: PaymentStatusPageProps) {
  ...
  useEffect(() => {
    ...
    // First poll: wait 1.5s to let mock webhook process; subsequent polls: 2s
    const delay = attempts === 0 ? 1500 : 2000;
  ...
  if (status === "COMPLETED" && isMock) {
    return (
      <div className="text-center py-8">
        ...
        <h2 className="text-xl font-bold text-slate-900 mb-2">Demo Booking Complete</h2>
        <p className="text-slate-500 mb-6 leading-relaxed">
          You just made a booking in demo mode — it is a simulation so you can explore how GoRASA works.
          This booking is saved and ready to go. Once you sign in, it will show up in My Bookings right
          where you would expect it.
        </p>
        <button
          onClick={handleExitDemo}
          disabled={exiting}
          className="inline-block px-6 py-3 bg-brand-saffron text-white rounded-xl font-bold hover:bg-brand-burnt transition-colors disabled:opacity-50 cursor-pointer"
        >
          {exiting ? "Signing out..." : "Go to My Bookings"}
        </button>
      </div>
    );
  }
